...
class Server:
    ...
    def handle_message(self, conn):
        while True:  # Boucle infinie, surveillance continue des messages clients
            try:
                data = conn.recv(4096)  # Accepter les données envoyées par le client, jusqu'à 2048 octets
                if not data:  # S'il n'y a pas de données, la connexion peut être fermée
                    ...
                    print(self.get_other_players_data(data["id"]))
    ...
    def update_one_player_data(self, data):
        ...
        #position = data["id"][0]
        #frame_num = data["id"][1]
        #current_dir = data["id"][2]
        position = data["position"]  
        frame_num = data["frame_num"]
        current_dir = data["current_dir"]
        self.players_data[player_id] = (position, frame_num, current_dir)
